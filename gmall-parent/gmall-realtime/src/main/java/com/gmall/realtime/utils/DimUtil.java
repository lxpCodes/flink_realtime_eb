...
public class DimUtil {
    ...
    static {
        cache = CacheBuilder.newBuilder().maximumSize(10000).expireAfterWrite(1, TimeUnit.MINUTES).recordStats().build();
    }
    ...
    */
    ...
    public static JSONObject getDimInfo(String tableName, Tuple2<String, String>... cloNameAndValue) {
        ...
        try {
            ...
            //根据key到Redis中查询
//            dimJsonStr = cache.getIfPresent(redisKey);
            dimJsonStr = jedis.get(redisKey);
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("从guava cache中查询维度失败");
        }
...
//                cache.put(redisKey,dimJsonObj.toJSONString());
                if (jedis != null) {
...
//        JSONObject dimInfo = DimUtil.getDimInfoNoCache("DIM_BASE_TRADEMARK", Tuple2.of("id", "14"));
        ...
        JSONObject dimInfo = DimUtil.getDimInfoNoCache("BASE_TRADEMARK", Tuple2.of("id","14"));
        System.out.println(dimInfo);

        cache.put("test","sadsdadadaadadad");
